...
class ExceptionHandler(commands.Cog):
    ...
    async def on_command_error(self, ctx, error: commands.CommandError):
        ...
        if isinstance(error, commands.NoPrivateMessage):
            try:
                await ctx.author.send(
                    f"⚠️ {str(ctx.command).upper()} cannot be used in Private Messages."
                )
        ...
        if isinstance(error, Forbidden):
            embed = discord.Embed(
                title="🖐️ Hold it...",
                description=f"⁉️ Required permission is missing or unattainable.",
        ...
        if isinstance(error, commands.MissingRequiredArgument):
            embed = discord.Embed(
                title="⚠️ Unable to proceed...",
                description=f"Vital argument `{error.param.name}` is missing.",
                color=Color.dark_red(),
            )
